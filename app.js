...
function bindEvents() {
  ...
  document.getElementById("triggerFilter").addEventListener("change", (event) => {
...
function hydrateCurrentWeek() {
  ...
  state.priorWeek = priorLabel ? state.store.weeks[priorLabel] || null : null;
...
function renderOverview() {
  ...
  const priorSummary = state.priorWeek?.summary || null;
  context.textContent = priorSummary
...
function renderFlowTable() {
  ...
  if (!state.filteredRows.length) {
    tbody.innerHTML = `<tr><td class="empty-cell" colspan="${tableColumns.length}">No flows match the current search or trigger filter.</td></tr>`;
    return;
  }
...
function renderLineSvg(series, metric) {
  ...
  const xTicks = points.map((point, index) => `<text class="tick-label" x="${scaleX(index)}" y="${height - 18}" text-anchor="middle">${escapeHtml(point.label)}</text>`).join("");
  const yTicks = [0, 0.25, 0.5, 0.75, 1].map((factor) => {
...
function sortRows(rows, sortKey, direction) {
  const column = tableColumns.find((item) => item.key === sortKey);
...
function getSortableValue(value, type) {
  if (type.startsWith("wow")) {
    return value?.sortValue ?? Number.NEGATIVE_INFINITY;
  }
  return value;
}
...
function extractWeekLabel(fileName) {
  const matched = String(fileName || "").match(/(week\s*\d+\s*-\s*\d+)/i);
  return matched ? normalizeWeekLabel(matched[1]) : "";
}
...
function normalizeWeekLabel(value) {
  ...
  if (matched) {
    return `week${Number(matched[1])}-${Number(matched[2])}`;
  }
  return raw;
}
...
function sortWeekLabels(labels) {
  return [...new Set(labels)].sort((a, b) => {
    ...
    if (parsedA && parsedB) {
      return parsedA.start - parsedB.start || parsedA.end - parsedB.end;
    }
    if (parsedA) {
      return -1;
    }
